...
?>
  ...
  <div id="formControls">
    ...
    <?php $this->txtIdServicio->RenderWithName(); ?>
    ...
    <?php $this->txtPrecio->RenderWithName(); ?><?php $this->lstTipoMonedaPrecio->Render(); ?>
		...
		<?php
					...
					$ficha_sql = "
						SELECT precio_desde
						FROM fichas 
						WHERE id =".$this->intIdFichas."
					";
					$ficha = $database->list_assoc($ficha_sql);
                    $database->close();
					
		?>
		<div style="display:inline;" id="c596_ctll">
			<div class="renderWithName"><div class="left"><label for="c596">Precio desde</label></div>
				<div class="right">
					<input type="checkbox" name="precio_desde" value="1" <?=($ficha[0]['precio_desde']==1?"checked":"");?>/>
				</div>
			</div>
		</div>						
...
//            }
    ...
    <?php $this->lstIdEstadoObject->RenderWithName(); ?>
        ...
        <?php if ($this->lstImagenPrincipal) : ?>
            <h3>Elegir Imagen Principal</h3>
            <?php $this->lstImagenPrincipal->Render(); ?>
        <?php endif; ?>
        ...
        <?php $this->lstAsociarNota->RenderWithName(); ?>
				...
				<?php
					...
					$servicio_sql = "
						...
						ORDER BY nombre ASC
					";
					$servicios = $database->list_assoc($servicio_sql);
					
					//echo $this->txtIdServicio->Text;

				?>

				<div style="display:inline;" id="c596_ctll">
					<div class="renderWithName"><div class="left"><label for="c596">Asociar a un servicio</label></div>
						<div class="right">
							<select size="1" class="listbox"  name="id_servicio">
								<option value="">--</option>
								<? foreach($servicios as $servicio) : ?>
								<option value="<?=$servicio['id_servicio'];?>" <?php echo $servicio['id_servicio']==$this->txtIdServicio->Text?"selected='selected'":''; ?> ><?=$servicio['id_servicio'];?> - <?=$servicio['nombre'];?></option>
								<? endforeach; ?>				
							</select>
						</div>
					</div>
        ...
        <?php $this->lstAsociarNotaImagenes->RenderWithName(); ?>
            ...
            <h3>Asociar Video</h3>
        <?php $this->lstAsociarVideo->RenderWithName(); ?>

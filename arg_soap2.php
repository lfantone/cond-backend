...
/*
 */
class ExampleSoap extends QSoapService {
    ...
    public function GetFichaXml($intId) {
        ...
        $objXmlConduciendoPanel = new XmlConduciendoPanel($this, $intId);
    ...
    public function ConsultarNovedades($intCantidad, $strFecha) {
        ...
        try {
            ...
            $objDbResult = $objDatabase->Query("SELECT f.id, f2.accion, f2.fecha, UNIX_TIMESTAMP(f2.fecha) as fecha_unix FROM fichas_info f2 INNER JOIN fichas f on f.id = f2.id_fichas ".
                    " WHERE fecha >= ".$objDatabase->SqlVariable($strFecha)." AND ( procesada IS NULL or procesada = 0 ) AND id_pais = 2 ". #AND f2.accion != 'baja'
...
ExampleSoap::Run('ExampleSoap', 'http://conduciendo.com/backend/example_soap.php');
?>

...
if($_POST){	
  $titulo = $_POST['titulo'];
	...
	$cargar_nota = $database->insert("magazine_notas","titulo = '$titulo', subtitulo = '$subtitulo', copete = '$copete', contenido = '$contenido', estado = 2, id_autor = $id_autor, fecha_creada = NOW() ");
	$id_nota = $database->insert_id();
	header("Location: magazine_notas_editar.php?id=$id_nota");
}
...
$notas = $database->row_assoc("SELECT COUNT(id) cantidad FROM magazine_notas WHERE estado <> 1 AND titulo LIKE '%$keyword%'");
...
$notas_sql = "
	SELECT magazine_notas.id, magazine_notas.titulo, estados.nombre estado FROM magazine_notas INNER JOIN estados ON estados.id = magazine_notas.estado WHERE magazine_notas.titulo LIKE '%$keyword%' AND magazine_notas.estado <> 1 ORDER BY magazine_notas.id DESC LIMIT $desde,60
";
...
function updateLinkXml(id) {
  if (id > 0) {
    ...
    $('linkxml').writeAttribute('href','/backend/magazine_archivos_xml_generar.php?id='+id+'&target='+id);
  } else {
...
<div class="container">
	<div class="content">
		<h1>Magazine Notas (&asymp; <?=$cantidad;?>)</h1>
		<div class="options">
			<ul>
		...
		<div class="grid">
        		...
        		<form action="magazine_generate_newsml.php" method="post">
			<table width="100%" border="0" cellspacing="0" cellpadding="6">
				...
				<tr>
					...
					<td><a href="magazine_notas_editar.php?id=<?=$nota['id'];?>">
						<?=$nota['titulo'];?>
						</a></td>
					<td><?=$nota['estado'];?></td>
					<td><a href="magazine_notas_borrar.php?id=<?=$nota['id'];?>"><img src="assets/images/close.gif" width="13" height="14" border="0" /></a></td>
				</tr>
				<? } ?>				
				<!--<tr>
					<td>&nbsp;</td>
...
?>	
		<tr>
			<td colspan='3'>
				  <button type="submit">Generar NEWSML</button></td>
			</td>
		</tr>
       <!--<tr>
          <td>&nbsp;</td>
          ...
          <td>&nbsp;</td>
				...
				<tr>
					<td colspan="4" align="center"><div class="paginate"> <a href="magazine_notas.php?pagina=1&keyword=<?=$keyword;?>">primera p&aacute;gina</a>
								<? for($i=$extremo_izquierdo;$i<=$extremo_derecho;$i++){?>
								<? if($pagina==$i){ ?>
								<a class="active" href="magazine_notas.php?pagina=<?=$i;?>&keyword=<?=$keyword;?>">
									<?=$i;?>
							</a>
								<? } else { ?>
								<a href="magazine_notas.php?pagina=<?=$i;?>&keyword=<?=$keyword;?>">
									<?=$i;?>
							</a>
								<? } ?>
								<? } ?>
							<a href="magazine_notas.php?pagina=<?=$paginas;?>&keyword=<?=$keyword;?>">ultima p&aacute;gina</a> </div></td>
				</tr>
				<? } else { ?>
				<tr class="empty">

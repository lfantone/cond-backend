<?php
	 ...
	 * listing a collection of Tipos objects.  It includes
	 ...
	 * 
	 */
	class TiposDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Tipos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Tipos object, or it can be any QQNode extending from QQN::Tipos().
		 * 
		 ...
		 * sorting using two fields from the Tipos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Tipos
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Tipos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Tipos, given the clauses above
			$this->DataSource = Tipos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Tipos-based QQNode.
		 * It will also verify that it is a proper Tipos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tipos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tipos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Tipos()->Id;
				case 'Nombre': return QQN::Tipos()->Nombre;
				default: throw new QCallerException('Simple Property not found in TiposDataGrid content: ' . $mixContent);

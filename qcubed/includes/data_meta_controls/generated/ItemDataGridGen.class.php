<?php
	 ...
	 * listing a collection of Item objects.  It includes
	 ...
	 * 
	 */
	class ItemDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Item datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Item object, or it can be any QQNode extending from QQN::Item().
		 * 
		 ...
		 * sorting using two fields from the Item object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Item
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdItem=<?=urlencode($_ITEM->IdItem)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdItem)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdItem, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Item::CountAll();
			}
			...
			$this->DataSource = Item::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Item-based QQNode.
		 * It will also verify that it is a proper Item-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'item') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "item".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdItem': return QQN::Item()->IdItem;
				case 'Nombre': return QQN::Item()->Nombre;
				case 'Descripcion': return QQN::Item()->Descripcion;
				case 'Seccion': return QQN::Item()->Seccion;
				case 'OrdenSeccion': return QQN::Item()->OrdenSeccion;
				case 'IdTipoDatoType': return QQN::Item()->IdTipoDatoType;
				case 'IdPais': return QQN::Item()->IdPais;
				case 'IdPaisObject': return QQN::Item()->IdPaisObject;
				default: throw new QCallerException('Simple Property not found in ItemDataGrid content: ' . $mixContent);

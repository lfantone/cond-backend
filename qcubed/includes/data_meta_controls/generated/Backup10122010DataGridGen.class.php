<?php
	 ...
	 * listing a collection of Backup10122010 objects.  It includes
	 ...
	 * 
	 */
	class Backup10122010DataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Backup10122010 datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Backup10122010 object, or it can be any QQNode extending from QQN::Backup10122010().
		 * 
		 ...
		 * sorting using two fields from the Backup10122010 object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Backup10122010
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdModelo, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Backup10122010::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Backup10122010, given the clauses above
			$this->DataSource = Backup10122010::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Backup10122010-based QQNode.
		 * It will also verify that it is a proper Backup10122010-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'backup10122010') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "backup10122010".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdModelo': return QQN::Backup10122010()->IdModelo;
				case 'Nombre': return QQN::Backup10122010()->Nombre;
				case 'IdMarca': return QQN::Backup10122010()->IdMarca;
				default: throw new QCallerException('Simple Property not found in Backup10122010DataGrid content: ' . $mixContent);

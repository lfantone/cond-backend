<?php
	 ...
	 * listing a collection of FichasPaises objects.  It includes
	 ...
	 * 
	 */
	class FichasPaisesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasPaises datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasPaises object, or it can be any QQNode extending from QQN::FichasPaises().
		 * 
		 ...
		 * sorting using two fields from the FichasPaises object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasPaises
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasPaises::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasPaises, given the clauses above
			$this->DataSource = FichasPaises::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasPaises-based QQNode.
		 * It will also verify that it is a proper FichasPaises-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_paises') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_paises".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::FichasPaises()->Id;
				case 'IdFicha': return QQN::FichasPaises()->IdFicha;
				case 'IdPais': return QQN::FichasPaises()->IdPais;
				case 'IdPaisObject': return QQN::FichasPaises()->IdPaisObject;
				default: throw new QCallerException('Simple Property not found in FichasPaisesDataGrid content: ' . $mixContent);

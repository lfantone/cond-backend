<?php
	 ...
	 * listing a collection of NotasArchivos objects.  It includes
	 ...
	 * 
	 */
	class NotasArchivosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasArchivos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasArchivos object, or it can be any QQNode extending from QQN::NotasArchivos().
		 * 
		 ...
		 * sorting using two fields from the NotasArchivos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasArchivos
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasArchivos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasArchivos, given the clauses above
			$this->DataSource = NotasArchivos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasArchivos-based QQNode.
		 * It will also verify that it is a proper NotasArchivos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_archivos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_archivos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasArchivos()->Id;
				case 'IdNota': return QQN::NotasArchivos()->IdNota;
				case 'IdArchivo': return QQN::NotasArchivos()->IdArchivo;
				default: throw new QCallerException('Simple Property not found in NotasArchivosDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of Videos objects.  It includes
	 ...
	 * 
	 */
	class VideosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Videos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Videos object, or it can be any QQNode extending from QQN::Videos().
		 * 
		 ...
		 * sorting using two fields from the Videos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Videos
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Videos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Videos, given the clauses above
			$this->DataSource = Videos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Videos-based QQNode.
		 * It will also verify that it is a proper Videos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'videos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "videos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Videos()->Id;
				case 'Titulo': return QQN::Videos()->Titulo;
				case 'Subtitulo': return QQN::Videos()->Subtitulo;
				case 'Contenido': return QQN::Videos()->Contenido;
				case 'Fecha': return QQN::Videos()->Fecha;
				case 'Estado': return QQN::Videos()->Estado;
				default: throw new QCallerException('Simple Property not found in VideosDataGrid content: ' . $mixContent);

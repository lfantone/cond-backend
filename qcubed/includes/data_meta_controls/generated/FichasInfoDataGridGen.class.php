<?php
	 ...
	 * listing a collection of FichasInfo objects.  It includes
	 ...
	 * 
	 */
	class FichasInfoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasInfo datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasInfo object, or it can be any QQNode extending from QQN::FichasInfo().
		 * 
		 ...
		 * sorting using two fields from the FichasInfo object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasInfo
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdFichasInfo=<?=urlencode($_ITEM->IdFichasInfo)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdFichasInfo)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdFichasInfo, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasInfo::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasInfo, given the clauses above
			$this->DataSource = FichasInfo::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasInfo-based QQNode.
		 * It will also verify that it is a proper FichasInfo-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_info') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_info".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdFichasInfo': return QQN::FichasInfo()->IdFichasInfo;
				case 'IdFichas': return QQN::FichasInfo()->IdFichas;
				case 'IdFichasObject': return QQN::FichasInfo()->IdFichasObject;
				case 'Fecha': return QQN::FichasInfo()->Fecha;
				case 'Accion': return QQN::FichasInfo()->Accion;
				case 'Notas': return QQN::FichasInfo()->Notas;
				default: throw new QCallerException('Simple Property not found in FichasInfoDataGrid content: ' . $mixContent);

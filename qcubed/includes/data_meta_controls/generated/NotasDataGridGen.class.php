<?php
	 ...
	 * listing a collection of Notas objects.  It includes
	 ...
	 * 
	 */
	class NotasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Notas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Notas object, or it can be any QQNode extending from QQN::Notas().
		 * 
		 ...
		 * sorting using two fields from the Notas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Notas
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Notas::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Notas, given the clauses above
			$this->DataSource = Notas::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Notas-based QQNode.
		 * It will also verify that it is a proper Notas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Notas()->Id;
				case 'Titulo': return QQN::Notas()->Titulo;
				case 'Subtitulo': return QQN::Notas()->Subtitulo;
				case 'Copete': return QQN::Notas()->Copete;
				case 'Contenido': return QQN::Notas()->Contenido;
				case 'ContenidoRtf': return QQN::Notas()->ContenidoRtf;
				case 'FechaCreada': return QQN::Notas()->FechaCreada;
				case 'FechaPublicada': return QQN::Notas()->FechaPublicada;
				case 'Visitas': return QQN::Notas()->Visitas;
				case 'IdAutor': return QQN::Notas()->IdAutor;
				case 'Estado': return QQN::Notas()->Estado;
				case 'FechaAPublicar': return QQN::Notas()->FechaAPublicar;
				default: throw new QCallerException('Simple Property not found in NotasDataGrid content: ' . $mixContent);

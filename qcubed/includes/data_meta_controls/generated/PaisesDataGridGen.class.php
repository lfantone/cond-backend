<?php
	 ...
	 * listing a collection of Paises objects.  It includes
	 ...
	 * 
	 */
	class PaisesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Paises datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Paises object, or it can be any QQNode extending from QQN::Paises().
		 * 
		 ...
		 * sorting using two fields from the Paises object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Paises
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Paises::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Paises, given the clauses above
			$this->DataSource = Paises::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Paises-based QQNode.
		 * It will also verify that it is a proper Paises-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'paises') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "paises".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Paises()->Id;
				case 'Nombre': return QQN::Paises()->Nombre;
				default: throw new QCallerException('Simple Property not found in PaisesDataGrid content: ' . $mixContent);

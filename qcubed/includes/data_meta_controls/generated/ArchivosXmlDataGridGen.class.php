<?php
	 ...
	 * listing a collection of ArchivosXml objects.  It includes
	 ...
	 * 
	 */
	class ArchivosXmlDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ArchivosXml datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ArchivosXml object, or it can be any QQNode extending from QQN::ArchivosXml().
		 * 
		 ...
		 * sorting using two fields from the ArchivosXml object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ArchivosXml
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ArchivosXml::CountAll();
			}
			...
			// Set the DataSource to be a Query result from ArchivosXml, given the clauses above
			$this->DataSource = ArchivosXml::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ArchivosXml-based QQNode.
		 * It will also verify that it is a proper ArchivosXml-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'archivos_xml') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "archivos_xml".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ArchivosXml()->Id;
				case 'IdEmpresa': return QQN::ArchivosXml()->IdEmpresa;
				case 'Fecha': return QQN::ArchivosXml()->Fecha;
				case 'Estado': return QQN::ArchivosXml()->Estado;
				case 'Nombre': return QQN::ArchivosXml()->Nombre;
				default: throw new QCallerException('Simple Property not found in ArchivosXmlDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of MonedaPais objects.  It includes
	 ...
	 * 
	 */
	class MonedaPaisDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MonedaPais datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MonedaPais object, or it can be any QQNode extending from QQN::MonedaPais().
		 * 
		 ...
		 * sorting using two fields from the MonedaPais object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MonedaPais
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdMoneda=<?=urlencode($_ITEM->IdMoneda)?>&intIdPais=<?=urlencode($_ITEM->IdPais)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdMoneda)?>/<?=urlencode($_ITEM->IdPais)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdMoneda . "," . $_ITEM->IdPais, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = MonedaPais::CountAll();
			}
			...
			// Set the DataSource to be a Query result from MonedaPais, given the clauses above
			$this->DataSource = MonedaPais::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MonedaPais-based QQNode.
		 * It will also verify that it is a proper MonedaPais-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'moneda_pais') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "moneda_pais".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdMoneda': return QQN::MonedaPais()->IdMoneda;
				case 'IdPais': return QQN::MonedaPais()->IdPais;
				case 'Orden': return QQN::MonedaPais()->Orden;
				default: throw new QCallerException('Simple Property not found in MonedaPaisDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of Moneda objects.  It includes
	 ...
	 * 
	 */
	class MonedaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Moneda datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Moneda object, or it can be any QQNode extending from QQN::Moneda().
		 * 
		 ...
		 * sorting using two fields from the Moneda object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Moneda
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdMoneda=<?=urlencode($_ITEM->IdMoneda)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdMoneda)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdMoneda, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Moneda::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Moneda, given the clauses above
			$this->DataSource = Moneda::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Moneda-based QQNode.
		 * It will also verify that it is a proper Moneda-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'moneda') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "moneda".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdMoneda': return QQN::Moneda()->IdMoneda;
				case 'Nombre': return QQN::Moneda()->Nombre;
				default: throw new QCallerException('Simple Property not found in MonedaDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of FichasNotas objects.  It includes
	 ...
	 * 
	 */
	class FichasNotasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasNotas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasNotas object, or it can be any QQNode extending from QQN::FichasNotas().
		 * 
		 ...
		 * sorting using two fields from the FichasNotas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasNotas
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdFichaNota=<?=urlencode($_ITEM->IdFichaNota)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdFichaNota)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdFichaNota, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasNotas::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasNotas, given the clauses above
			$this->DataSource = FichasNotas::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasNotas-based QQNode.
		 * It will also verify that it is a proper FichasNotas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_notas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_notas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdFichaNota': return QQN::FichasNotas()->IdFichaNota;
				case 'IdFichas': return QQN::FichasNotas()->IdFichas;
				case 'IdFichasObject': return QQN::FichasNotas()->IdFichasObject;
				case 'IdNota': return QQN::FichasNotas()->IdNota;
				default: throw new QCallerException('Simple Property not found in FichasNotasDataGrid content: ' . $mixContent);

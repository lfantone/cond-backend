<?php
	 ...
	 * listing a collection of NotasTipos objects.  It includes
	 ...
	 * 
	 */
	class NotasTiposDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasTipos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasTipos object, or it can be any QQNode extending from QQN::NotasTipos().
		 * 
		 ...
		 * sorting using two fields from the NotasTipos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasTipos
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasTipos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasTipos, given the clauses above
			$this->DataSource = NotasTipos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasTipos-based QQNode.
		 * It will also verify that it is a proper NotasTipos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_tipos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_tipos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasTipos()->Id;
				case 'IdNota': return QQN::NotasTipos()->IdNota;
				case 'IdTipo': return QQN::NotasTipos()->IdTipo;
				default: throw new QCallerException('Simple Property not found in NotasTiposDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of FichasImagenes objects.  It includes
	 ...
	 * 
	 */
	class FichasImagenesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasImagenes datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasImagenes object, or it can be any QQNode extending from QQN::FichasImagenes().
		 * 
		 ...
		 * sorting using two fields from the FichasImagenes object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasImagenes
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasImagenes::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasImagenes, given the clauses above
			$this->DataSource = FichasImagenes::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasImagenes-based QQNode.
		 * It will also verify that it is a proper FichasImagenes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_imagenes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_imagenes".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::FichasImagenes()->Id;
				case 'IdFicha': return QQN::FichasImagenes()->IdFicha;
				case 'IdFichaObject': return QQN::FichasImagenes()->IdFichaObject;
				case 'IdImagen': return QQN::FichasImagenes()->IdImagen;
				case 'IdImagenObject': return QQN::FichasImagenes()->IdImagenObject;
				case 'ImagenPrincipal': return QQN::FichasImagenes()->ImagenPrincipal;
				default: throw new QCallerException('Simple Property not found in FichasImagenesDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of FichasItemValor objects.  It includes
	 ...
	 * 
	 */
	class FichasItemValorDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasItemValor datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasItemValor object, or it can be any QQNode extending from QQN::FichasItemValor().
		 * 
		 ...
		 * sorting using two fields from the FichasItemValor object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasItemValor
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdFichas=<?=urlencode($_ITEM->IdFichas)?>&intIdItem=<?=urlencode($_ITEM->IdItem)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdFichas)?>/<?=urlencode($_ITEM->IdItem)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdFichas . "," . $_ITEM->IdItem, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasItemValor::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasItemValor, given the clauses above
			$this->DataSource = FichasItemValor::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasItemValor-based QQNode.
		 * It will also verify that it is a proper FichasItemValor-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_item_valor') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_item_valor".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdFichas': return QQN::FichasItemValor()->IdFichas;
				case 'IdFichasObject': return QQN::FichasItemValor()->IdFichasObject;
				case 'IdItem': return QQN::FichasItemValor()->IdItem;
				case 'IdItemObject': return QQN::FichasItemValor()->IdItemObject;
				case 'Valor': return QQN::FichasItemValor()->Valor;
				default: throw new QCallerException('Simple Property not found in FichasItemValorDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of Imagenes objects.  It includes
	 ...
	 * 
	 */
	class ImagenesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Imagenes datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Imagenes object, or it can be any QQNode extending from QQN::Imagenes().
		 * 
		 ...
		 * sorting using two fields from the Imagenes object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Imagenes
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Imagenes::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Imagenes, given the clauses above
			$this->DataSource = Imagenes::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Imagenes-based QQNode.
		 * It will also verify that it is a proper Imagenes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'imagenes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "imagenes".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Imagenes()->Id;
				case 'Titulo': return QQN::Imagenes()->Titulo;
				case 'Fecha': return QQN::Imagenes()->Fecha;
				case 'Estado': return QQN::Imagenes()->Estado;
				default: throw new QCallerException('Simple Property not found in ImagenesDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of ModeloBackup objects.  It includes
	 ...
	 * 
	 */
	class ModeloBackupDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ModeloBackup datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ModeloBackup object, or it can be any QQNode extending from QQN::ModeloBackup().
		 * 
		 ...
		 * sorting using two fields from the ModeloBackup object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ModeloBackup
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ModeloBackup::CountAll();
			}
			...
			// Set the DataSource to be a Query result from ModeloBackup, given the clauses above
			$this->DataSource = ModeloBackup::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ModeloBackup-based QQNode.
		 * It will also verify that it is a proper ModeloBackup-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'modelo_backup') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "modelo_backup".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdModelo': return QQN::ModeloBackup()->IdModelo;
				case 'Nombre': return QQN::ModeloBackup()->Nombre;
				case 'IdMarca': return QQN::ModeloBackup()->IdMarca;
				default: throw new QCallerException('Simple Property not found in ModeloBackupDataGrid content: ' . $mixContent);

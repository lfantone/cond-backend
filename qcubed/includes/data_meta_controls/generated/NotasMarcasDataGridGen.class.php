<?php
	 ...
	 * listing a collection of NotasMarcas objects.  It includes
	 ...
	 * 
	 */
	class NotasMarcasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasMarcas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasMarcas object, or it can be any QQNode extending from QQN::NotasMarcas().
		 * 
		 ...
		 * sorting using two fields from the NotasMarcas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasMarcas
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasMarcas::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasMarcas, given the clauses above
			$this->DataSource = NotasMarcas::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasMarcas-based QQNode.
		 * It will also verify that it is a proper NotasMarcas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_marcas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_marcas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasMarcas()->Id;
				case 'IdNota': return QQN::NotasMarcas()->IdNota;
				case 'IdMarca': return QQN::NotasMarcas()->IdMarca;
				default: throw new QCallerException('Simple Property not found in NotasMarcasDataGrid content: ' . $mixContent);

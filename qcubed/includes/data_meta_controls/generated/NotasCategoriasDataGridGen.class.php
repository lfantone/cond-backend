<?php
	 ...
	 * listing a collection of NotasCategorias objects.  It includes
	 ...
	 * 
	 */
	class NotasCategoriasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasCategorias datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasCategorias object, or it can be any QQNode extending from QQN::NotasCategorias().
		 * 
		 ...
		 * sorting using two fields from the NotasCategorias object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasCategorias
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasCategorias::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasCategorias, given the clauses above
			$this->DataSource = NotasCategorias::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasCategorias-based QQNode.
		 * It will also verify that it is a proper NotasCategorias-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_categorias') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_categorias".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasCategorias()->Id;
				case 'IdNota': return QQN::NotasCategorias()->IdNota;
				case 'IdCategoria': return QQN::NotasCategorias()->IdCategoria;
				case 'IdCategoriaObject': return QQN::NotasCategorias()->IdCategoriaObject;
				default: throw new QCallerException('Simple Property not found in NotasCategoriasDataGrid content: ' . $mixContent);

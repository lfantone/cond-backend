<?php
	 ...
	 * listing a collection of Estados objects.  It includes
	 ...
	 * 
	 */
	class EstadosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Estados datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Estados object, or it can be any QQNode extending from QQN::Estados().
		 * 
		 ...
		 * sorting using two fields from the Estados object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Estados
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Estados::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Estados, given the clauses above
			$this->DataSource = Estados::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Estados-based QQNode.
		 * It will also verify that it is a proper Estados-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'estados') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "estados".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Estados()->Id;
				case 'Nombre': return QQN::Estados()->Nombre;
				default: throw new QCallerException('Simple Property not found in EstadosDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of Autores objects.  It includes
	 ...
	 * 
	 */
	class AutoresDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Autores datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Autores object, or it can be any QQNode extending from QQN::Autores().
		 * 
		 ...
		 * sorting using two fields from the Autores object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Autores
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Autores::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Autores, given the clauses above
			$this->DataSource = Autores::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Autores-based QQNode.
		 * It will also verify that it is a proper Autores-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'autores') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "autores".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Autores()->Id;
				case 'Nombre': return QQN::Autores()->Nombre;
				case 'Email': return QQN::Autores()->Email;
				case 'Password': return QQN::Autores()->Password;
				case 'IdGrupo': return QQN::Autores()->IdGrupo;
				case 'Superadmin': return QQN::Autores()->Superadmin;
				default: throw new QCallerException('Simple Property not found in AutoresDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of NotasImagenes objects.  It includes
	 ...
	 * 
	 */
	class NotasImagenesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasImagenes datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasImagenes object, or it can be any QQNode extending from QQN::NotasImagenes().
		 * 
		 ...
		 * sorting using two fields from the NotasImagenes object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasImagenes
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasImagenes::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasImagenes, given the clauses above
			$this->DataSource = NotasImagenes::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasImagenes-based QQNode.
		 * It will also verify that it is a proper NotasImagenes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_imagenes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_imagenes".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasImagenes()->Id;
				case 'IdNota': return QQN::NotasImagenes()->IdNota;
				case 'IdImagen': return QQN::NotasImagenes()->IdImagen;
				case 'IdImagenObject': return QQN::NotasImagenes()->IdImagenObject;
				case 'Predeterminada': return QQN::NotasImagenes()->Predeterminada;
				default: throw new QCallerException('Simple Property not found in NotasImagenesDataGrid content: ' . $mixContent);

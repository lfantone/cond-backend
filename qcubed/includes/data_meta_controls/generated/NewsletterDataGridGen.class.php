<?php
	 ...
	 * listing a collection of Newsletter objects.  It includes
	 ...
	 * 
	 */
	class NewsletterDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Newsletter datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Newsletter object, or it can be any QQNode extending from QQN::Newsletter().
		 * 
		 ...
		 * sorting using two fields from the Newsletter object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Newsletter
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Newsletter::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Newsletter, given the clauses above
			$this->DataSource = Newsletter::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Newsletter-based QQNode.
		 * It will also verify that it is a proper Newsletter-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'newsletter') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "newsletter".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Newsletter()->Id;
				case 'Email': return QQN::Newsletter()->Email;
				case 'Fecha': return QQN::Newsletter()->Fecha;
				case 'Estado': return QQN::Newsletter()->Estado;
				default: throw new QCallerException('Simple Property not found in NewsletterDataGrid content: ' . $mixContent);

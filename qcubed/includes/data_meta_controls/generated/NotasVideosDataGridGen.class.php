<?php
	 ...
	 * listing a collection of NotasVideos objects.  It includes
	 ...
	 * 
	 */
	class NotasVideosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasVideos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasVideos object, or it can be any QQNode extending from QQN::NotasVideos().
		 * 
		 ...
		 * sorting using two fields from the NotasVideos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasVideos
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasVideos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasVideos, given the clauses above
			$this->DataSource = NotasVideos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasVideos-based QQNode.
		 * It will also verify that it is a proper NotasVideos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_videos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_videos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasVideos()->Id;
				case 'IdNota': return QQN::NotasVideos()->IdNota;
				case 'IdVideo': return QQN::NotasVideos()->IdVideo;
				default: throw new QCallerException('Simple Property not found in NotasVideosDataGrid content: ' . $mixContent);

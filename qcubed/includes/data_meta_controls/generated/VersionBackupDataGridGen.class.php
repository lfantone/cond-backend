<?php
	 ...
	 * listing a collection of VersionBackup objects.  It includes
	 ...
	 * 
	 */
	class VersionBackupDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this VersionBackup datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the VersionBackup object, or it can be any QQNode extending from QQN::VersionBackup().
		 * 
		 ...
		 * sorting using two fields from the VersionBackup object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from VersionBackup
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = VersionBackup::CountAll();
			}
			...
			// Set the DataSource to be a Query result from VersionBackup, given the clauses above
			$this->DataSource = VersionBackup::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a VersionBackup-based QQNode.
		 * It will also verify that it is a proper VersionBackup-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'version_backup') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "version_backup".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdVersion': return QQN::VersionBackup()->IdVersion;
				case 'Nombre': return QQN::VersionBackup()->Nombre;
				case 'IdModelo': return QQN::VersionBackup()->IdModelo;
				default: throw new QCallerException('Simple Property not found in VersionBackupDataGrid content: ' . $mixContent);

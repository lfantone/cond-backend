<?php
	 ...
	 * listing a collection of NotasPaises objects.  It includes
	 ...
	 * 
	 */
	class NotasPaisesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasPaises datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasPaises object, or it can be any QQNode extending from QQN::NotasPaises().
		 * 
		 ...
		 * sorting using two fields from the NotasPaises object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasPaises
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasPaises::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasPaises, given the clauses above
			$this->DataSource = NotasPaises::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasPaises-based QQNode.
		 * It will also verify that it is a proper NotasPaises-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_paises') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_paises".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasPaises()->Id;
				case 'IdNota': return QQN::NotasPaises()->IdNota;
				case 'IdPais': return QQN::NotasPaises()->IdPais;
				case 'IdPaisObject': return QQN::NotasPaises()->IdPaisObject;
				default: throw new QCallerException('Simple Property not found in NotasPaisesDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of Marcas objects.  It includes
	 ...
	 * 
	 */
	class MarcasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Marcas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Marcas object, or it can be any QQNode extending from QQN::Marcas().
		 * 
		 ...
		 * sorting using two fields from the Marcas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Marcas
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Marcas::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Marcas, given the clauses above
			$this->DataSource = Marcas::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Marcas-based QQNode.
		 * It will also verify that it is a proper Marcas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'marcas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "marcas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Marcas()->Id;
				case 'Nombre': return QQN::Marcas()->Nombre;
				default: throw new QCallerException('Simple Property not found in MarcasDataGrid content: ' . $mixContent);

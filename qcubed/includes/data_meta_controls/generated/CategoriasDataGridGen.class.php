<?php
	 ...
	 * listing a collection of Categorias objects.  It includes
	 ...
	 * 
	 */
	class CategoriasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Categorias datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Categorias object, or it can be any QQNode extending from QQN::Categorias().
		 * 
		 ...
		 * sorting using two fields from the Categorias object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Categorias
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Categorias::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Categorias, given the clauses above
			$this->DataSource = Categorias::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Categorias-based QQNode.
		 * It will also verify that it is a proper Categorias-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'categorias') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "categorias".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Categorias()->Id;
				case 'Nombre': return QQN::Categorias()->Nombre;
				default: throw new QCallerException('Simple Property not found in CategoriasDataGrid content: ' . $mixContent);

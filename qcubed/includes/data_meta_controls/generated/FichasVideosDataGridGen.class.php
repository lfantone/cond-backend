<?php
	 ...
	 * listing a collection of FichasVideos objects.  It includes
	 ...
	 * 
	 */
	class FichasVideosDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FichasVideos datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FichasVideos object, or it can be any QQNode extending from QQN::FichasVideos().
		 * 
		 ...
		 * sorting using two fields from the FichasVideos object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FichasVideos
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdFichaVideo=<?=urlencode($_ITEM->IdFichaVideo)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdFichaVideo)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdFichaVideo, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FichasVideos::CountAll();
			}
			...
			// Set the DataSource to be a Query result from FichasVideos, given the clauses above
			$this->DataSource = FichasVideos::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FichasVideos-based QQNode.
		 * It will also verify that it is a proper FichasVideos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fichas_videos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fichas_videos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdFichaVideo': return QQN::FichasVideos()->IdFichaVideo;
				case 'IdFicha': return QQN::FichasVideos()->IdFicha;
				case 'IdFichaObject': return QQN::FichasVideos()->IdFichaObject;
				case 'IdVideo': return QQN::FichasVideos()->IdVideo;
				case 'IdVideoObject': return QQN::FichasVideos()->IdVideoObject;
				default: throw new QCallerException('Simple Property not found in FichasVideosDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of MarcasBackup objects.  It includes
	 ...
	 * 
	 */
	class MarcasBackupDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MarcasBackup datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MarcasBackup object, or it can be any QQNode extending from QQN::MarcasBackup().
		 * 
		 ...
		 * sorting using two fields from the MarcasBackup object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MarcasBackup
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = MarcasBackup::CountAll();
			}
			...
			// Set the DataSource to be a Query result from MarcasBackup, given the clauses above
			$this->DataSource = MarcasBackup::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MarcasBackup-based QQNode.
		 * It will also verify that it is a proper MarcasBackup-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'marcas_backup') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "marcas_backup".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::MarcasBackup()->Id;
				case 'Nombre': return QQN::MarcasBackup()->Nombre;
				default: throw new QCallerException('Simple Property not found in MarcasBackupDataGrid content: ' . $mixContent);

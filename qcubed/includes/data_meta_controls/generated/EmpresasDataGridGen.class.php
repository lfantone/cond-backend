<?php
	 ...
	 * listing a collection of Empresas objects.  It includes
	 ...
	 * 
	 */
	class EmpresasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Empresas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Empresas object, or it can be any QQNode extending from QQN::Empresas().
		 * 
		 ...
		 * sorting using two fields from the Empresas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Empresas
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Empresas::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Empresas, given the clauses above
			$this->DataSource = Empresas::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Empresas-based QQNode.
		 * It will also verify that it is a proper Empresas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'empresas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "empresas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Empresas()->Id;
				case 'Nombre': return QQN::Empresas()->Nombre;
				default: throw new QCallerException('Simple Property not found in EmpresasDataGrid content: ' . $mixContent);

<?php
	 ...
	 * listing a collection of NotasArchivosOld objects.  It includes
	 ...
	 * 
	 */
	class NotasArchivosOldDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this NotasArchivosOld datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NotasArchivosOld object, or it can be any QQNode extending from QQN::NotasArchivosOld().
		 * 
		 ...
		 * sorting using two fields from the NotasArchivosOld object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NotasArchivosOld
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NotasArchivosOld::CountAll();
			}
			...
			// Set the DataSource to be a Query result from NotasArchivosOld, given the clauses above
			$this->DataSource = NotasArchivosOld::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NotasArchivosOld-based QQNode.
		 * It will also verify that it is a proper NotasArchivosOld-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'notas_archivos_old') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "notas_archivos_old".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::NotasArchivosOld()->Id;
				case 'IdNota': return QQN::NotasArchivosOld()->IdNota;
				case 'IdArchivo': return QQN::NotasArchivosOld()->IdArchivo;
				default: throw new QCallerException('Simple Property not found in NotasArchivosOldDataGrid content: ' . $mixContent);

...
class QFilteredDataGridColumn extends QDataGridColumn {
	...
	public function FilterSetOperand($mixOperand) {
		try {
			...
			elseif($this->objFilter instanceof QQConditionLogical){
                            foreach ($this->objFilter->objConditionArray as $objQQComparison){
                                $objQQComparison->mixOperand = $this->strFilterPrefix . $mixOperand . $this->strFilterPostfix;
                            }

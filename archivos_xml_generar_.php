...
if ($target == 5) {
		for ($i=0;$i<count($paquete);$i++) {
			...
			$paquete[$i]['titulo']		= utf8_encode($paquete[$i]['titulo']);
			$paquete[$i]['subtitulo'] = utf8_encode($paquete[$i]['subtitulo']);
			$paquete[$i]['copete']		= utf8_encode($paquete[$i]['copete']);
		}
    $strXml = put_entities(msn_xml($paquete));
...
if($target==2){
    ...
    $strXml = mercadolibre_xml($paquete);
		echo $strXml;

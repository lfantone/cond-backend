...
if ($target == 5) {
    for ($i=0;$i<count($paquete);$i++) {
      // esto es necesario porque hay diferencias de encoding entre estos campos y el contenido principal
      // (el contenido esta en utf8 y el resto no)

      $paquete[$i]['titulo']		= utf8_encode(remove_spaces($paquete[$i]['titulo']));
      $paquete[$i]['subtitulo'] = utf8_encode(remove_spaces($paquete[$i]['subtitulo']));
      $paquete[$i]['copete']		= utf8_encode(remove_spaces($paquete[$i]['copete']));
    }
    $strXml = put_entities(Encoding::fixUTF8(msn_xml($paquete)));

    if ($_GET['pais_id']==3) {// mexico
      @file_put_contents(dirname(__FILE__)."/../msnxml/newsml_mex.xml", $strXml);
    } else {
...
if($target==1){
  ...
  echo put_entities(Encoding::fixUTF8(formatear_texto(yahoo_xml($paquete))));
}
if($target==2){
    ...
    $strXml = Encoding::fixUTF8(mercadolibre_xml($paquete));
    echo $strXml;

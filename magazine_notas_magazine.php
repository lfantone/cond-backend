...
if($_POST){	
  $titulo = $_POST['titulo'];
	...
	$cargar_nota = $database->insert("notas_magazine","titulo = '$titulo', subtitulo = '$subtitulo', copete = '$copete', contenido = '$contenido', estado = 2, id_autor = $id_autor, fecha_creada = NOW() ");
...
$notas = $database->row_assoc("SELECT COUNT(id) cantidad FROM notas_magazine WHERE estado <> 1 AND titulo LIKE '%$keyword%'");
...
$notas_sql = "
	SELECT notas.id, notas.titulo, estados.nombre estado FROM notas_magazine INNER JOIN estados ON estados.id = notas.estado WHERE notas.titulo LIKE '%$keyword%' AND notas.estado <> 1 ORDER BY notas.id DESC LIMIT $desde,60
";
...
<div class="container">
	<div class="content">
		...
		<div class="grid">
        		...
        		<form action="magazine_notas_xml_agregar.php" method="post">

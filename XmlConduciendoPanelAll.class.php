...
class XmlConduciendoPanelAll {

    protected $objXml;
    protected $intIdPais;
    protected $intIdFichas;
    public $objXmlArray = array();

    public function Render($blnDisplayOutput = true) {
        $strReturn = '';
        foreach ($this->objXmlArray as $objXml) {
            $strReturn .= $objXml->Render($blnDisplayOutput, true);
        }
        return $strReturn;
    }

    public function __construct($objParentObject, $intIdPais, $strControlId = null) {
        $this->intIdPais = $intIdPais;
        
        $objFichasArray = Fichas::QueryArray(
            QQ::Equal(QQN::Fichas()->IdPais, $this->intIdPais)
        );
       
        if (count($objFichasArray))
        foreach ($objFichasArray as $objFichas) { 
            $this->intIdFichas = $objFichas->Id;
            ...
            $objNodoMarca->AddChildElement("Nombre", $objFichas->IdMarcaObject->Nombre);
            ...
            if (is_array($objFichasNotasArray)) {
                foreach ($objFichasNotasArray as $objFichasNotas) {
                    ...
                    $strTitulo = iconv("ISO-8859-1", "UTF-8", $objFichasNotas->IdNotaObject->Titulo);
                    $strCopete = iconv("ISO-8859-1", "UTF-8", $objFichasNotas->IdNotaObject->Copete);
                    $strContenido = iconv("ISO-8859-1", "UTF-8", $objFichasNotas->IdNotaObject->Contenido);
            ...
            array_push($this->objXmlArray, $this->objXml);
